...
int Server::on_connect(int sockfd, struct sockaddr_in *client_addr)
{
    ...
    Session *session = m_session_mgr->get_session(sockfd,addr,this->m_session_life);
    if (!session)
    {
        close(sockfd);
        return -1;
...
int Server::run()
{
  //  epoll_add(m_listen_fd, EPOLLIN | EPOLLPRI, &m_listen_fd);
	epoll_add(m_listen_fd, EPOLLIN , &m_listen_fd);
    for (;;)
    {
        ...
        for (int i = 0; i < nfds; i++)
        {
            ...
            {
#ifdef DEBUG
            //	logdbg("process(%d) has data coming in \n",getpid());
#endif
            	Analysisi::TestTimeAnalysis sis(1);
