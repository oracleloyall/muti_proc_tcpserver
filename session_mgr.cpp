...
#include<iostream>
int total=0;
Session *SessionMgr::get_session(u32 fd,u32 nip,int64_t s_life)
{
    map<u32, Session *>::iterator it;
#ifdef  DEBUG
   // cout<<"sockfd  is:"<<fd<<endl;
    it = m_session_map.find(fd);
...
Session *SessionMgr::get_session(u32 nip,int64_t s_life)
{
    map<u32, Session *>::iterator it;
#ifdef  DEBUG
    cout<<"toal is:"<<total++<<endl;
    it = m_session_map.find(total);
        if (it != m_session_map.end())
        {
            return it->second;
        }
...
void SessionMgr::add_session(u32 fd,Session *session)
{
    m_session_map.insert(pair<u32, Session *>(fd, session));
}
