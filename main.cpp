...
int pullchild(){
   ...
   int  m_ep_fd = epoll_create(5000);
#ifdef DEBUG
   struct epoll_event * m_ep_events = (struct epoll_event *)calloc(5000, sizeof(struct epoll_event));
#else
    //modify by zhaoxi
    m_ep_events = (struct epoll_event *)malloc((m_ep_max_poll, sizeof(struct epoll_event))*1000);
#endif
    if (!m_ep_events)
    {
    ...
    {
    	logdbg("master !\n");
        //need to add g_fifo_fd to epool
    }
    else
    {
    	logdbg("new child create!\n");
    }
}
...
{
	 stringstream ss;
	 ss<<"killall  muti_proc_tcp";
	 int rc = system(ss.str().c_str());
	 if (rc != 0)
	 {
	    logerr("system cmd:%s failed!\n", ss.str().c_str());
	 }
	 printf("killall  muti_proc_tcp\n");
	 exit(-1);
	 return ;
}
static int run_main()
